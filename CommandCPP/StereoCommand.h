...
public:
	StereoOnWithCDCommand(std::shared_ptr<Stereo> stereo) { this->stereo = stereo; }
...
class StereoOnWithRadioCommand : public Command
{
	std::shared_ptr<Stereo> stereo = nullptr;
public:
	StereoOnWithRadioCommand(std::shared_ptr<Stereo> stereo) { this->stereo = stereo; }
	void execute() override
	{
		stereo->on();
		stereo->setRadio();
...
public:
	StereoOffCommand(std::shared_ptr<Stereo> stereo) { this->stereo = stereo; }
	void execute() override 
	{
